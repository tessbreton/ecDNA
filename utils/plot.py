...
from utils.other import dict_to_values, get_best_indices
...
def plot_histograms_dict_overlay(dictionaries:list[dict], bin_size=10, 
                                 ...
                                 plot_all=True, plot_ref=False, true_data=None, true_label='Reference data', ref_color='palegreen',
    ...
    if plot_ref:
        ...
        fig.add_trace(go.Histogram(x=true_data, name=true_label, marker_color=ref_color, opacity=0.7, 
                                    xbins=dict(size=bin_size), histnorm=histnorm, marker_pattern_shape=""))
...
def plot_ecdfs(distributions:list,
    ...
    for distribution, label, color in zip(distributions, labels, colors):
        ...
        fig.add_trace(go.Scatter(
            x=x, y=y, mode='lines', name=label, line=dict(color=color),
        ))

    fig.update_layout(
        title=title, xaxis_title='ecDNA copy number', yaxis_title='probability',
...
def plot_mean_CN_multi_simulations(n_simulations, filepath='meanmultiple.png', n_events=1000, 
                                   fitness='log', s=0.1, size=1000, plot_bgcolor='#F1F6FA',
                                   show=True, save_fig=False, width=1000, height=400, scale=5):
    fig = go.Figure()
    ...
    for i in tqdm(range(n_simulations)):
        ...
        fig.add_trace(go.Scatter(x=list(range(n_events)), y=mean_ecDNA_copy_numbers,
                                 mode='lines',
    ...
    fig.update_layout(
        ...
        xaxis=dict(title='Event count'), yaxis=dict(title='Mean ecDNA copy number'), plot_bgcolor=plot_bgcolor,
...
def plot_posterior(s_values, distances, top_percent=5, prior_color='lightskyblue', posterior_color='#FF005E',
                   show=True, save=False, filepath=None, add_ref=False, sref=None, width=700, height=400, scale=5):   
    ...
    fig = go.Figure()
    fig.add_trace(go.Histogram(x=s_values, name='Sampled', marker_color=prior_color, opacity=0.5, 
                               xbins=dict(size=0.005), histnorm='probability density', marker_pattern_shape=""))
    fig.add_trace(go.Histogram(x=top_s_values, name=f'Top {top_percent}%', marker_color=posterior_color, opacity=0.5, 
                               xbins=dict(size=0.005), histnorm='probability density', marker_pattern_shape=""))
...
def error_s_heatmap(error_s, s_intervals, start_values):
    ...
    fig = go.Figure(data=go.Heatmap(z=100 * error_s,
    ...
    fig.update_layout(
        ...
        yaxis=dict(tickvals=start_values, ticktext=start_values)
    )
    fig.show()
...
def qqplot(simulation, reference, title, show=True, save=False, filepath=None):
    ...
    fig.add_trace(go.Scatter(x=values1, y=values2, mode='markers',
                             name='QQ Plot', marker=dict(size=5, color='blue')))
    ...
    fig.update_layout(
        title=title, xaxis_title='Quantiles of best simulation',
...
def plot_best_points_util(s_values, distances, metric='Wasserstein', top_percent=5, best_color='turquoise',
    ...
    fig.add_trace(go.Scatter(x=s_values, y=distances, mode='markers', marker=dict(color='black', size=3), name='All points'))
    fig.add_trace(go.Scatter(x=np.array(s_values)[smallest_indices], y=np.array(distances)[smallest_indices], mode='markers',
                             marker=dict(color=best_color, size=5), name=f'{top_percent}% smallest distances'))
...
def plot_histogram_dict(dictionary,bin_size=5, plot_color='deepskyblue', plot_bgcolor='white', 
    ...
    layout = go.Layout(title=title, xaxis=dict(title='Number of copies'), bargap=0.1, plot_bgcolor=plot_bgcolor)
